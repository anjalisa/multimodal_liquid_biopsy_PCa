...
class RowChecker:
	...
	third_col="FASTQ_UMI",
        ...
        **kwargs,
	    ...
	    third_col (str): The name of the column that contains the third (if any)
                FASTQ file path (default "FASTQ_UMI").
            single_col (str): The name of the new column that will be inserted and
                records whether the sample contains single- or paired-end sequencing
                reads (default "paired_end").
    ...
    def _validate_pair(self, row):
        ...
        if row[self._first_col] and row[self._second_col] and row[self._third_col]:
            ...
            assert (
                Path(row[self._first_col]).suffixes[-2:] == Path(row[self._second_col]).suffixes[-2:] == Path(row[self._third_col]).suffixes[-2:]
