...
for(bam in bamfiles){
  ...
  # Run coverage analysis #
  cr <- MEDIPS.seqCoverage(file = bamfiles[1], pattern = "CG", BSgenome = "BSgenome.Hsapiens.UCSC.hg19", chr.select = paste0("chr", c(1:22)), uniq = 0, paired = paired_end)
  ...
  table <- data.frame(Sample_ID = gsub("*.final.bam", "", bamfiles[1]), CpGs.covered = cov, CpGs.greater.5 = cov.data.greater.5, reads.WO.CPG = cov.data.0) 
  
  x <- 2:length(bamfiles)
  for(i in seq(along=x)){
    y <- i + 1  
    
    cr <- MEDIPS.seqCoverage(file = bamfiles[y], pattern = "CG", BSgenome = "BSgenome.Hsapiens.UCSC.hg19", chr.select = paste0("chr", c(1:22)), uniq = 0, paired = paired_end)
    ...
    table <- rbind(table, data.frame(Sample_ID = gsub("*.final.bam", "", bamfiles[y]), CpGs.covered = cov, CpGs.greater.5 = cov.data.greater.5, reads.WO.CPG = cov.data.0))  
