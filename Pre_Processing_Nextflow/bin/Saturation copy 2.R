...
for(bam in bamfiles){
  ...
  sr <- MEDIPS.saturation(file = bamfiles[1], BSgenome = "BSgenome.Hsapiens.UCSC.hg19", uniq = 0, window_size = window_size, paired = paired_end, chr.select = paste0("chr", c(1:22)))
  
  sat.data <- data.frame(Sample_ID = gsub("*.final.bam", "", bamfiles[1]), saturation.r = sr$maxEstCor[2], read.cov = sr$maxEstCor[1])
  
  x <- 2:length(bamfiles)
  for(i in seq(along=x)){
    y <- i + 1 
    
    sr <- MEDIPS.saturation(file = bamfiles[y], BSgenome = "BSgenome.Hsapiens.UCSC.hg19", uniq = 0, window_size = window_size, paired = paired_end, chr.select = paste0("chr", c(1:22)))
    sat.data <- rbind(sat.data, data.frame(Sample_ID = gsub("*.final.bam", "", bamfiles[y]), saturation.r = sr$maxEstCor[2], read.cov = sr$maxEstCor[1])) 
