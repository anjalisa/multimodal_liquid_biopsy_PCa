...
# Last updated: 2022/03/02 ; 2022/07/11 (nextflow version)
#
# Description
#   (1) Summarizes quality metrics
# --------------------------------------------------------------------------------------------------
...
if (!require("matrixStats")){
  install.packages("matrixStats" , dependencies=TRUE, repos='http://cloud.r-project.org/')
}
...
saturation <- args[1]       # Directory of input files (= QC results)
coverage <- args[2]
enrichment <- args[3]
read_count <- args[4]
fastqc <- args[5]
METHYL <- args[6]     # params.skip_methylation_qc

# Check whether Methylation QC was performed or not
methyl_skip <- if(METHYL=='SKIPPED') TRUE else FALSE

## Combine quality metrics ##
# Get list of files to combine #
files <- list.files(saturation, coverage, enrichment, read_count, fastqc)
...
for(i in 1:length(files)){
  ...
  tmp <- data.frame(fread(file.path(files[i])))
...
# Save #
summary <- table
fwrite(summary, file = "QC_summary.tsv", sep = "\t")
...
# Save summary table #
fwrite(summary.2, file = "Metric_summary.tsv", sep = "\t")
...
ggsave(
  filename = "Coverage_overview.svg",
...
ggsave(
  filename = "Coverage_overview.png",
  plot = ggplot2::last_plot(),
  device = "png",
  scale = 1.25,
  width = 14,
...
if(methyl_skip == FALSE){
  ...
  ggsave(
    filename = "Saturation_overview.svg",
    plot = ggplot2::last_plot(),
    device = "png",
    scale = 1.25,
    width = 12,
  ...
  ggsave(
    filename = "Saturation_overview.png",
    plot = ggplot2::last_plot(),
    device = "png",
    scale = 1.25,
    width = 12,
