...
option_list <- list(
  ...
  make_option(c("--gender"), type = "character", default=NULL, help = "Manually define gender of tumour patients, in case they have all the same gender and this gender is known. Default: [%default]"),
  make_option(c("--genderNormal"), type = "character", default=NULL, help = "Manually define gender of control cohort, in case all controls have the same gender and this gender is known. Default: [%default]")
)
...
for (i in 1:numSamples) {
  ...
  if(gender == "female"){
    gender <- list(gender="female")
  } 

  if (is.null(gender) || gender == "None" || gender == "NULL"){
  ...
  if (!is.null(normal_file) && normal_file != "None" && normal_file != "NULL"){
    ...
    if(genderNormal == "male"){
      gender.normal <- list(gender="male")
    } 

    if(genderNormal == "female"){
      gender.normal <- list(gender="female")
    } 

    if (is.null(genderNormal) || genderNormal == "None" || genderNormal == "NULL"){
...
for (i in 1:numSamples) {
  ...
  #path.in <- file.path(outDir, paste0(id))
    ...
    params <- data.frame(fread(file.path(paste0(id, ".params.txt")), sep = ";"))
    params <- params[c(6:7), ]
    ...
    cna <- data.frame(fread(file.path(paste0(id, ".cna.seg"))))
    txt <- data.frame(fread(file.path(paste0(id, ".seg.txt"))))
    ...
    #cna <- cna[!cna$chr == "X", ]        ## adapted by Anja (# added)
    ...
    chr$chr <-  sub("chr*", "", chr$chr)    ## added by anja
    chr$chr <- factor(chr$chr, levels=c(seq(1:22), "X", "Y")) # changed by Anja
    chr <- chr[order(chr$chr), ]
    chr$counts_half <- chr$count / 2
    chr$cumsum <- cumsum(chr$count)
    chr$tick <- chr$cumsum -chr$count + chr$counts_half
    
    cna$chr <- sub("chr*", "", cna$chr)    ## added by anja
    ...
    #!!! changed by Anja: c(seq(1:22),"X", "Y")
    ggplot() +
